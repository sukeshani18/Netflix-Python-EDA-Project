# Netflix Data Analysis Using Python

## Project Overview
This project performs Exploratory Data Analysis (EDA) on Netflix content data to uncover insights related to movies, TV shows, ratings, genres, countries, and content trends over time.

The goal is to understand Netflix’s content strategy using data.

---

## Dataset Information
- Source: Netflix Titles Dataset
- Contains information about movies and TV shows on Netflix
- Includes: title, type, director, cast, country, release year, rating, duration, and genre

---

## Tools & Libraries Used
- Python
- Pandas
- NumPy
- Matplotlib

---

## Project Workflow

### 1. Data Cleaning
- Handled missing values
- Removed duplicates
- Standardized dataset

### 2. Exploratory Data Analysis (EDA)
- Movies vs TV Shows distribution
- Top countries producing content
- Content growth over years
- Ratings analysis
- Genre distribution

### 3. Visualization
- Bar charts
- Line chart
- Pie chart

---

## 📈 Key Insights

- Movies dominate Netflix content compared to TV Shows
- United States is the largest content contributor
- Netflix expanded rapidly after 2015
- TV-MA is the most common rating
- Drama and international content are highly popular genres

---

## Conclusion
This project demonstrates data analysis skills including data cleaning, visualization, and business insight generation using Python.

It helps understand how Netflix has evolved its content strategy over time.

---

## Author
Raksh Balpande
Aspiring Data Analyst
